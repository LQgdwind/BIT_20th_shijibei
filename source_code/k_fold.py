import source_code.merge_dataset as md
import torch
import torchvision
...
def k_fold(num, batch_size = 128, num_k =10):
    ...
    if num_k == 10:
        train_valid_ds = []
        for i in range(10):
            train_valid_ds.append(torchvision.datasets.ImageFolder("..//traindata//train" + str(i),
                                                                   transform=transform))
        if num != 0:
            train_ds = torchvision.datasets.ImageFolder("..//traindata//train0", transform=transform)
            for i in range(9):
                if num == i + 1:
                    continue
                else:
            ...
            train_ds = torchvision.datasets.ImageFolder("..//traindata//train1",
                                                        transform=transform)
            for i in range(8):
                md.merge(train_ds, train_valid_ds[i + 2])

        valid_ds = train_valid_ds[num]
        test_ds = torchvision.datasets.ImageFolder(".//testdata", transform=transform)
        ...
        if num != 0:
            ...
            train_ds = torchvision.datasets.ImageFolder("..//traindata//train2",
                                                        transform=transform)
            for i in range(7):
